...
import usr.logging.*;
import usr.console.RestCommand;
 ...
 */
public class CommandAsRestHandler extends USRRequestHandler  {
    ...
    public boolean handle(Request request, Response response) {
        try {
            ...
            ManagementConsole console = getManagementConsole();
            NEMSkin skin = (NEMSkin)console;
            RestCommand command = (RestCommand)((ConsoleSupport)skin).find(commandName);
            boolean result;

            //System.out.println(getClass().getName() + " FOUND command: " + command);

            if (command != null) {
                // we got a command
            } else {
                //fetch the UnknownCommand
                command = (RestCommand)((ConsoleSupport)skin).find("__UNKNOWN__");
            }
