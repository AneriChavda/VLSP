...
public class ControlOptions {
    ...
    void readXML(String fName) {
        try { DocumentBuilderFactory docBuilderFactory = DocumentBuilderFactory.newInstance();
            ...
            try {
                Logger.getLogger("log").logln(USR.ERROR, "Cannot find file "+fName + " in directory " + new java.io.File( "." ).getCanonicalPath() + " or " + System.getProperty("user.dir"));
            } catch (Exception fe) {
            } finally {
                System.exit(-1);
            }
    ...
    private void processGlobalController(NodeList gc) throws SAXException {
        ...
        try {
         ...
         try {
             warmUpPeriod_ = ReadXMLUtils.parseSingleInt(gcn, "WarmUpPeriod", "GlobalController", true) * 1000;
        ...
        if (monitoring.getLength() != 0) {
            ...
            Logger.getLogger("log").logln(USR.STDOUT, leadin() + "No GlobalController Monitoring node");
        }
    ...
    private EventEngine processEventEngine(Node n) throws SAXException {
        ...
        try {
            ...
            throw new SAXException("Could not find engine type " + engine + " " + e.getMessage());
        }
