    ...
    public void deleteLink(Request request, Response response) throws IOException, JSONException {
        ...
        if (sc.hasNextInt()) {
            ...
            if (gc.isValidLinkID(id)) {
                ...
                // delete a link
                gc.endLink(System.currentTimeMillis(), 
                           (Integer)li.getEndPoints().getFirst(), (Integer)li.getEndPoints().getSecond());
    ...
    public void setLinkAttributes(Request request, Response response) throws IOException, JSONException {
        ...
        if (sc.hasNextInt()) {
            ...
            if (gc.isValidLinkID(id)) {
                ...
                // delete a link
                gc.setLinkWeight(System.currentTimeMillis(), (Integer)li.getEndPoints().getFirst(),
                                    (Integer)li.getEndPoints().getSecond(), weight);

                // and send them back as the return value
                PrintStream out = response.getPrintStream();

                JSONObject jsobj = gc.findLinkInfoAsJSON(id);
