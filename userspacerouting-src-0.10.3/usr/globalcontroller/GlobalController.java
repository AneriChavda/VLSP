...
public class GlobalController implements ComponentController {
    ...
    LocalHostInfo myHostInfo_;  // Information about the local info
    private String gcAddress = "localhost"; // Current host
    ...
    // Main listening port for the console
    int mainPort;
    ...
    private ArrayList<Integer> routerList_ = null;   // List of router IDs (integers) which
    ...
    protected boolean init() {
        try {
            ...
            try {
                myHostInfo_ = new LocalHostInfo(getPort());
            } catch (Exception e) {
            ...
            if (latticeMonitoring) {
                /* set up consumers of router monitoring data */
                ...
                try {
                    ...
                    // check if gcAddress == myHostInfo_.getName()
                    System.err.println("gcAddress = " + gcAddress);
                    System.err.println("myHostInfo_.getIp() = " + myHostInfo_.getIp());
            ...
            Logger.getLogger("log").logln(USR.ERROR, leadin()+ "init failed");
            return false;
        }
    ...
    private void setupReporters(HashMap<String, String> reporterInfoMap) {
        ...
        for (Map.Entry<String, String> entry : reporterInfoMap.entrySet()) {
            ...
            try {
                ...
                Constructor<? extends Reporter> cons = (Constructor<? extends Reporter> )cc.getDeclaredConstructor(
    ...
    protected void startConsole() {
        console_ = new GlobalControllerManagementConsole(this, getPort());
        console_.start();
    }
            ...
            if (type == SimEvent.EVENT_START_SIMULATION) {
                ...
                if (arg instanceof Integer) {
                    ...
                    if (rInfo == null) {
                        Logger.getLogger("log").logln(USR.ERROR,
                                                      leadin() + "Unknown router " + routerAddress +" in END_ROUTER at time " +
                                                      time);
                        shutDown();
                    }
                ...
                if (data instanceof Pair) {
                    ...
                    if (pair.getFirst() instanceof Integer && pair.getSecond() instanceof Integer) {
                        ...
                        if (r1Info == null) {
                            Logger.getLogger("log").logln(USR.ERROR,
                                                          leadin() + "Unknown router " + router1Address +" in START_LINK at time " +
                                                          time);
                            shutDown();
                        }
                        ...
                        if (r2Info == null) {
                            Logger.getLogger("log").logln(USR.ERROR,
                                                          leadin() + "Unknown router " + router2Address +" in START_LINK at time " +
                                                          time);
                            shutDown();
                        }
                ...
                if (pair.getFirst() instanceof Integer && pair.getSecond() instanceof Integer) {
                    ...
                    if (r1Info == null) {
                        Logger.getLogger("log").logln(USR.ERROR,
                                                      leadin() + "Unknown router " + router1Address +" in END_LINK at time " +
                                                      time);
                        shutDown();
                    }
                    ...
                    if (r2Info == null) {
                        Logger.getLogger("log").logln(USR.ERROR,
                                                      leadin() + "Unknown router " + router2Address +" in END_LINK at time " +
                                                      time);
                        shutDown();
                    }
    ...
    throws IOException {
        ...
        try {
            ...
            Logger.getLogger("log").logln(USR.STDOUT, leadin() + "Creating router: " + id +
                                          (address !=
                                           null ? (" address = " + address) : "")  + (name != null ? (" name = " + name) : ""));

            // create the new router and get it's name
            routerAttrs = lci.newRouter(id, port, port+1, address, name);

            BasicRouterInfo br = new BasicRouterInfo((Integer)routerAttrs.get(
                                                                              "routerID"), getSimulationCurrentTime(), local, (Integer)routerAttrs.get("mgmtPort"),
            ...
            Logger.getLogger("log").logln(1<<9, elapsedToString(
                                              getSimulationElapsedTime()) + ANSI.GREEN + " START ROUTER " + id + ANSI.RESET_COLOUR);


            // inform anyone that a Router has Started
            informRouterStarted(routerAttrs);
    ...
    private void endVirtualRouter(int rId) {
        ...
        try {
            ...
            for (i = 0; i < MAX_TRIES; i++) {
                try {
                    routerAttrs = lci.endRouter(br.getHost(), br.getManagementPort());

                    Logger.getLogger("log").logln(1<<9, elapsedToString(
                                                      getSimulationElapsedTime()) + ANSI.RED + " STOP ROUTER " + rId +
                                                  ANSI.RESET_COLOUR);
            ...
            shutdownRouters_.add(new BasicRouterInfo(rId, getSimulationCurrentTime()));


            // inform anyone that a Router has Ended
            informRouterEnded(routerAttrs);

        } catch (JSONException e) {
            // Failed to end
            Logger.getLogger("log").logln(USR.ERROR, leadin() + "Could not end router " + rId + " on "+lci);
        }
    }
    ...
    protected void informRouterStarted(JSONObject routerAttrs) throws JSONException {
        ...
        if (latticeMonitoring) {
            String routerName = (String)routerAttrs.get("name");
    ...
    /** Register a link with structures necessary in Global
       Controller */
    private void registerLink(int router1Id, int router2Id, int weight) {
        noLinks_++;
        ...
        costs2[out.length] = weight; // Link cost 1 so far
        ...
        try {
            ...
            if (acquired) {
                ...
                if (gotIt) {             // we already have this link
                    ...
                    registerLink(router1Id, router2Id, weight);
    ...
    private int startVirtualLink(int router1Id, int router2Id, int weight, String name) {
        ...
        for (i = 0; i < MAX_TRIES; i++) {
            try {
                ...
                linkInfo.put(linkID, new LinkInfo(endPoints, connectionName, weight, linkID, getSimulationCurrentTime()));

                Logger.getLogger("log").logln(USR.STDOUT,
                                              leadin() + br1 + " -> " + br2 + " = " + connectionName + " with link ID: " + linkID);

                Logger.getLogger("log").logln(1<<9, elapsedToString(
                                                  getSimulationElapsedTime()) + ANSI.BLUE + " CREATE LINK " + router2Id + " TO " + router1Id +
                                              ANSI.RESET_COLOUR);
    ...
    private void endVirtualLink(int rId1, int rId2) {
        ...
        for (i = 0; i < MAX_TRIES; i++) {
            try {
                ...
                Logger.getLogger("log").logln(USR.STDOUT,
                                              leadin() + "remove link from: " + rId2 + " to " + rId1 + " with link ID: " + linkID);

                Logger.getLogger("log").logln(1<<9, elapsedToString(
                                                  getSimulationElapsedTime()) + ANSI.MAGENTA + " REMOVE LINK " + rId2 + " TO " + rId1 +
                                              ANSI.RESET_COLOUR);
    ...
    public int setLinkWeight(long time, int router1Id, int router2Id, int weight) {
        ...
        try {
            ...
            if (acquired) {
                ...
                setLinkWeight(router1Id, router2Id, weight);
    ...
    private int setLinkWeight(int router1Id, int router2Id, int weight) {
        ...
        //Logger.getLogger("log").logln(USR.STDOUT, "Got LC");
        lci1 = interactorMap_.get(lc1);
        lci2 = interactorMap_.get(lc2);
        //Logger.getLogger("log").logln(USR.STDOUT, "Got LCI");
        ...
        LinkInfo lInfo = findLinkInfo(linkID);
            ...
            if (acquired) {
                ...
                for (i = 0; i < MAX_TRIES; i++) {
                    try {
                        ...
                        // and set info as
                        // ["id": 46346535, "time" : "00:14:52", "aid" : 1, "startime" : 1331119233159, "state": "RUNNING",
                        ...
                        dataMap.put("StartTime", (Long)response.get("startTime"));
                        ...
                        Logger.getLogger("log").logln(USR.ERROR,
                                                      leadin()+
                                                      " failed to start app "+className+" on "+ routerID+ " try "+i + " with Exception " +
                                                      e);
    ...
    public int appStop(int routerID, int appID) {
        // return +ve no for valid id
        // return -1 for no stop - cant find LocalController
        // return -2 for no stop - router cant run app
        // return -3 for semaphore not acquired in time
        // return -4 for acquire interrupted
        try {
            // wait 2500ms to acquire the semaphore
    ...
    protected void shutDown() {
        ...
        if (isActive) {
            ...
            if (!options_.isSimulation()) {
                ...
                // stop all Routers
                long time = System.currentTimeMillis() - simulationStartTime;
                for (int routerId : new ArrayList<Integer>(routerList_)) {
                    int result = endRouter(time, routerId);
                }
    ...
    private void runRouterEvent(long time, SimEvent e) {
        ...
        if (sc.hasNextInt()) {
            ...
            if (rInfo == null) {
                Logger.getLogger("log").logln(USR.ERROR, leadin() + "Unknown router " + routerName +" in ON_ROUTER at time " +
                                              time);
                shutDown();
            }
    ...
    public synchronized void startMonitoringConsumer(InetSocketAddress addr) {
        ...
        Logger.getLogger("log").logln(USR.STDOUT, leadin()+ "Starting forwarding to " + forwardAddress);
    ...
    public long getSimulationCurrentTime() {
        ...
        if (current - simulationTime > 1000)  {   // more than 1 second out
    ...
    public long getSimulationElapsedTime() {
        return getSimulationCurrentTime() - simulationStartTime;
    }
    ...
    private void checkAllControllers() {
        ...
        for (tries = 0; tries < MAX_TRIES; tries++) {
            ...
            if (noControllers_ == localControllers_.size()) {
                Logger.getLogger("log").logln(USR.STDOUT, leadin() + "All LocalControllers connected after " +
                                              (tries+1) + " tries");
                isOK = true;
                break;
            }
    ...
    public void setAP(int gid, int AP) {
        ...
        // actually set the AP
        internalSetAP(gid, AP);
        //Logger.getLogger("log").logln(USR.STDOUT,leadin()+" router "+gid+ " now has access point "+AP);

        // print out a message
        if (gid == AP) {
            Logger.getLogger("log").logln(1<<8, elapsedToString(
                                              getSimulationElapsedTime()) + ANSI.BLUE + " ROUTER " + gid + " BECOME AP" +
                                          ANSI.RESET_COLOUR);
        } else {
            Logger.getLogger("log").logln(1<<8, elapsedToString(
                                              getSimulationElapsedTime()) + ANSI.CYAN + " ROUTER " + gid + " SET AP " + AP +
                                          ANSI.RESET_COLOUR);
        }
    ...
    public void apSet(int gid, int AP) {
        try {
            ...
            if (acquired) {
                ...
                if (gid == AP) {
                    Logger.getLogger("log").logln(1<<8, elapsedToString(
                                                      getSimulationElapsedTime()) + ANSI.RED + " ROUTER " + gid + " BECOME AP" +
                                                  ANSI.RESET_COLOUR);
                } else {
                    Logger.getLogger("log").logln(1<<8, elapsedToString(
                                                      getSimulationElapsedTime()) + ANSI.MAGENTA + " ROUTER " + gid + " SET AP " + AP +
                                                  ANSI.RESET_COLOUR);
                }
    ...
    private boolean internalSetAP(int gid, int AP) {
    ...
    public void setListenPort() {
        mainPort = setupListenPort();
        System.err.println(leadin() + " PORT = " + mainPort);
    }
